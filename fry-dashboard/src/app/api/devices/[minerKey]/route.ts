...
export async function GET(req: Request, { params }: { params: Promise<{ minerKey: string }> }) {
  ...
  try {
    ...
    if (!mongoUri) {
      await logger.error('MONGO_DASH_URI environment variable not set', 'get_device_details')
      return NextResponse.json({ error: 'Database configuration error' }, { status: 500 })
    }
    if (!mongoCaPath) {
      await logger.warn('MONGO_CA_CERT_PATH not set; TLS CA bundle may be missing for MongoDB', 'get_device_details')
    }
